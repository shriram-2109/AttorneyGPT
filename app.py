...
def convert_audio_to_text(audio_data, sample_rate, selected_language='English'):
    """Convert audio data to text with language support"""
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as temp_audio:
            ...
            with sr.AudioFile(temp_audio_path) as source:
                ...
                try:
                    language_code = LANGUAGE_CONFIG[selected_language]['code']
                    try:
                    ...
                    st.warning(f"Could not understand the audio in {selected_language}. Please speak clearly and try again.")
                    return None
...
with col2:
    st.image(r"Path-to-AttorneyGPTcover.png")
    ...
    try:
        ...
        with col2:
            if st.button("🎤 Record", key="record_button", 
                        help=f"Click to record your question in {st.session_state.selected_language} (5 seconds)"):
                ...
                if audio_data is not None:
                    ...
                    if transcribed_text:
                        st.info(f"Transcribed ({st.session_state.selected_language}): {transcribed_text}")
